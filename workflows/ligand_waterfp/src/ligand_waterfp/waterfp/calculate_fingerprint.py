...
def fp_from_density_profile(n_r, r_centers_nm, binwidth_nm, norm_tail_bins=NORM_TAIL_BINS_DEFAULT):
...
def fingerprints_from_rdf_table(rdf_df, binwidth_nm, norm_tail_bins=NORM_TAIL_BINS_DEFAULT):
    ...
    for atom, sub in rdf_df.groupby("atom", sort=False):
        ...
        fp, _, _ = fp_from_density_profile(sub["n_r"].to_numpy(), sub["r_nm"].to_numpy(), binwidth_nm, norm_tail_bins)
...
def parse_args():
    ...
    p.add_argument("--rdf-csv", required=True, help="Output of calculate_rdf.py (columns: atom,r_nm,n_r)")
